...
const OtherSetting = () => {
  ...
  const checkUpdate = async () => {
    try {
      ...
      //   `${proxyUrl}https://api.github.com/repos/QuantumNous/ai-bridge/releases/latest`,
      // );

      // Option 2: Use the JSON proxy approach which often works better with GitHub API
      const res = await fetch(
        'https://api.github.com/repos/QuantumNous/ai-bridge/releases/latest',
        {
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
            // Adding User-Agent which is often required by GitHub API
            'User-Agent': 'ai-bridge-update-checker',
          },
        },
  ...
  const openGitHubRelease = () => {
    window.open(
      `https://github.com/QuantumNous/ai-bridge/releases/tag/${updateData.tag_name}`,
      '_blank',
    );
  };
