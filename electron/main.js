...
function saveAndOpenErrorLog() {
  try {
    ...
    const logFileName = `aibridge-crash-${timestamp}.log`;
    ...
    // 写入日志
    const logContent = `AI Bridge 崩溃日志
...
function getBinaryPath() {
  ...
  switch (platform) {
    case 'win32':
      binaryName = 'ai-bridge.exe';
      break;
    case 'darwin':
      binaryName = 'ai-bridge';
      break;
    case 'linux':
      binaryName = 'ai-bridge';
      break;
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    env.SQLITE_PATH = path.join(dataDir, 'ai-bridge.db');
